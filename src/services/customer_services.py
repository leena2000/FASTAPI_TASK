...
def create_customer(db: Session, customer: Customer):
    db_customer = CustomerSchema(
        ...
        age=customer.age, gender=customer.gender, adult=customer.adult
    )
    db.add(db_customer)
    db.commit()
...
def update_customer(db: Session, id: UUID, customer: Customer):
    if db.query(CustomerSchema.id == id):
        db.query(CustomerSchema).filter(CustomerSchema.id == id)\
            .update(customer.dict(), synchronize_session=False)
        db.commit()
        updated = db.query(CustomerSchema).filter(CustomerSchema.id == id)\
                    .first()
        return updated


def delete_customer(db: Session, id: UUID):
    if db.query(CustomerSchema).filter(CustomerSchema.id == id)\
            .first() is None:
        return None
    ...
    db.commit()
    return {"message": f"deleted customer {id} successfully"}
