...
def create_address(db: Session, address: AddressIn, customer_id: UUID):
    db_address = AddressSchema(phone=address.phone, email=address.email,
...
def get_customer_address_customer_id(db: Session, id: UUID, skip: int = 0,
                                     limit: int = 100):
    return db.query(AddressSchema).filter(AddressSchema.customer_id == id).\
...
def update_customer_address(db: Session, id: UUID, address: Address):
    db.query(AddressSchema).filter(AddressSchema.id == id).\
        update(address.dict(), synchronize_session=False)
    db.commit()
    updated = db.query(AddressSchema).filter(AddressSchema.id == id)\
        .first()
    return updated
